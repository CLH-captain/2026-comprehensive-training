...
const editingStudent = ref<StudentDetail | null>(null)
...
function editStudent(): void {
  ...
  editingStudent.value = detail.value
  formOpen.value = true
}

async function deactivateStudent(): Promise<void> {
  if (!detail.value || !window.confirm(`确定停用学生“${detail.value.name}”吗？`)) return
  await http.delete(`/students/${detail.value.id}`)
  detail.value = null
  await loadStudents()
}

async function studentSaved(): Promise<void> {
  ...
  <div class="workspace">
    ...
    <main class="main-content">
      ...
      <section class="directory-panel">
        <header class="panel-header"><div><span class="section-tag">档案检索</span><h3>学生名单</h3></div><div class="panel-actions"><p>共 {{ numberFormatter.format(data?.total ?? 0) }} 条记录</p><button v-if="auth.isAdmin" type="button" @click="createStudent">＋ 新增学生</button></div></header>
        <div class="filters">
    ...
    <div v-if="detailLoading || detail" class="drawer-layer" @click.self="detail = null">
      <aside class="detail-drawer" aria-label="学生参与档案">
        ...
        <template v-else-if="detail">
          ...
          <div v-if="auth.isAdmin" class="profile-actions"><button type="button" @click="editStudent">编辑档案</button><button class="danger" type="button" @click="deactivateStudent">停用学生</button></div><section class="drawer-section"><header><h3>最近参与活动</h3><span>近 {{ detail.recent_activities.length }} 条</span></header><div class="activity-list"><div v-for="activity in detail.recent_activities" :key="activity.id"><time>{{ formatDate(activity.start_time).slice(5) }}</time><div><strong>{{ activity.title }}</strong><small>{{ activity.club_name }} · {{ activity.category_name }}</small></div><span :class="`attendance attendance--${activity.attendance_status}`">{{ activity.attendance_status === 'late' ? '迟到' : activity.attendance_status === 'present' ? '到场' : '缺席' }}</span></div></div></section>
        </template>
      </aside>
    </div>
  </div>
  <StudentFormDialog :open="formOpen" :student="editingStudent" :options="data?.options" @close="formOpen = false" @saved="studentSaved" />
</template>
